def naver_crawling(keyword):
    ...
    n_api_kin = f'https://openapi.naver.com/v1/search/kin.xml?query={keyword}'
    n_api_blog = f'https://openapi.naver.com/v1/search/blog.xml?query={keyword}'
    naver_api = [n_api_news, n_api_web, n_api_cafe, n_api_kin, n_api_blog]
...
def naver_search_trend(keyword):
  import requests
  import datetime
  import pandas as pd

  url = "https://openapi.naver.com/v1/datalab/search"
  headers = {"X-Naver-Client-Id" : "qh5A4Ihf6echaOazIsbF",
            "X-Naver-Client-Secret" : "1lbdDKlR55",
            "Content-Type" : "application/json" }

  body1 = """{"startDate":"2016-01-01","endDate":\""""
  body2 = """\","timeUnit":"month","keywordGroups":[{"groupName":"keyword","keywords":[\""""
  body3 = """\"]}]}"""

  today = str(datetime.datetime.today())[:10]
  body = body1 + today + body2 + keyword + body3
  ...
  for data in result['results'][0]['data']:
    db_lst.append([data['period'][:7], data['ratio']])
  db_df = pd.DataFrame(db_lst, columns=['year-month', 'ratio'])
